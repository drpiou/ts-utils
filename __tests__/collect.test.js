...
const { Collection, isCollection } = require('..');

test('@drpiou/ts-utils:collect', async () => {
  const source = ['a', 'b', 1];
  ...
  const chunkWhileA = result.chunkWhile((item, index, chunk) => chunk.contains((value) => value === item)).allDeep();
  ...
  testThis(result.all(), ['a', 'b', 1]);
  expect(typeof randomA).toBe('object');
  expect(randomA.length).toBe(1);
  expect(typeof randomB).toBe('object');
  expect(randomB.length).toBe(2);
  expect(typeof randomC).toBe('object');
  expect(randomC.length).toBe(5);
  expect(typeof randomD).toBe('object');
  expect(randomD.length).toBe(3);
  ...
  expect(typeof shuffleA).toBe('object');
  expect(shuffleA.length).toBe(2);
