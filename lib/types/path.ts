...
type WithParent<T, Key extends keyof T> = Key extends string
  ? Key | (T[Key] extends Record<string, any> ? `${Key}.${WithParent<T[Key], keyof T[Key]>}` : never)
  : never;

type WithoutParent<T, Key extends keyof T> = Key extends string
  ? never | (T[Key] extends Record<string, any> ? `${Key}.${WithoutParent<T[Key], keyof T[Key]>}` : Key)
  : never;

export type Path<T> = WithParent<T, keyof T>;

export type PathFinite<T> = WithoutParent<T, keyof T>;
